...
class TickStorage:
    ...
    def push_tick(self, selection_id: int, ltp: float, back_price: float, 
                  lay_price: float, back_size: float = 0, lay_size: float = 0,
    ...
    def get_spread_history(self, selection_id: int, limit: int = 100) -> List[float]:
        ...
        ticks = self.get_ticks(selection_id, limit)
        return [t.lay_price - t.back_price for t in ticks if t.back_price > 0 and t.lay_price > 0]
