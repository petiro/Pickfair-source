...
The core application logic is managed by `main.py`, integrating with `betfair_client.py` for API interactions and `telegram_listener.py` for Telegram chat monitoring and signal processing. `database.py` handles SQLite operations, while `dutching.py` manages complex dutching calculations. An `auto_updater.py` is in place for seamless updates. A `plugin_manager.py` offers a secure sandbox environment for extending functionality, complemented by `license_manager.py` for hardware-based license validation.
